...
void DbConnection::connect(
  ...
  if (client_flag & CLIENT_SSL) {
    ...
    // MySQL 8.0 deprecated the enforce SSL flag in prefernce to defining an SSL
    // MODE
    ...
    mysql_ssl_set(
      ...
      Rf_isNull(ssl_capath) ? NULL
                            : cpp11::as_cpp<std::string>(ssl_capath).c_str(),
      Rf_isNull(ssl_cipher) ? NULL
                            : cpp11::as_cpp<std::string>(ssl_cipher).c_str()
    );
#endif
    // From MySQL manual: Do not set this option within an application program;
    // it is set internally in the client library. Instead the application
    // program should set mysql_options
    client_flag &= ~CLIENT_SSL;
  }
  if (client_flag & CLIENT_SSL_VERIFY_SERVER_CERT) {
#if MYSQL_VERSION_ID >= 80000 && MYSQL_VERSION_ID < 100000
    // Note: in MySQL 8.0, there is only mode, so we "upgrade" the mode to
    // verify identity. Also, MySQL 8.0 has the ability to just verify the
    // server CA, but our interface doesn't define that. If that is the desired
    // SSL mode, then configure that via a defaults file or group definition.
  ...
  if (!Rf_isNull(ssl_key)) {
    mysql_options(
      this->pConn_,
      MYSQL_OPT_SSL_KEY,
      cpp11::as_cpp<std::string>(ssl_key).c_str()
    );
  }
  if (!Rf_isNull(ssl_cert)) {
    mysql_options(
      this->pConn_,
      MYSQL_OPT_SSL_CERT,
      cpp11::as_cpp<std::string>(ssl_cert).c_str()
    );
  }
  if (!Rf_isNull(ssl_ca)) {
    mysql_options(
      this->pConn_,
      MYSQL_OPT_SSL_CA,
      cpp11::as_cpp<std::string>(ssl_ca).c_str()
    );
  }
  ...
  if (!mysql_real_connect(
        ...
        Rf_isNull(password) ? NULL
                            : cpp11::as_cpp<std::string>(password).c_str(),
        Rf_isNull(db) ? NULL : cpp11::as_cpp<std::string>(db).c_str(),
        port,
        Rf_isNull(unix_socket)
          ? NULL
          : cpp11::as_cpp<std::string>(unix_socket).c_str(),
        client_flag
      )) {
    std::string error = mysql_error(this->pConn_);
    std::string hint(
      mysql_errno(this->pConn_) == 2026
        ? "\nTo suppress this try seting envvar "
...
cpp11::list DbConnection::info() {
  ...
  return cpp11::list(
    { "host"_nm = std::string(pConn_->host),
      ...
      "ssl.cipher"_nm = (ssl_cipher != NULL)
                          ? cpp11::as_sexp(std::string(ssl_cipher))
                          : R_NilValue,
...
SEXP DbConnection::quote_string(const cpp11::r_string& input) {
  ...
  size_t end =
    mysql_real_escape_string(pConn_, &output[1], input_str.c_str(), input_len);
...
void DbConnection::begin_transaction() {
  if (is_transacting()) {
    cpp11::stop("Nested transactions not supported.");
  }
  check_connection();
...
void DbConnection::commit() {
  if (!is_transacting()) {
    cpp11::stop("Call dbBegin() to start a transaction.");
  }
  check_connection();
